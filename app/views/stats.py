...
async def ot_stats_page(
    ...
    can_filter_team = is_admin
    effective_team = team if is_admin else current_user.get("team")
    effective_team_label = effective_team or "All Teams"
    ...
    if not is_admin:
        ...
        uids = await _team_user_ids(db, effective_team)
        q = q.where(OtRequest.user_id.in_(uids))
    ...
    if not is_admin:
        ...
        user_q = user_q.where(User.team == effective_team)
    ...
    usage_rows.sort(key=lambda x: -x["pct"])
    usage_scope_count = len(usage_rows)
    usage_list = usage_rows[:6]

    # Usage footer
    all_used = [u["used_hours"] for u in usage_rows]
    ...
    at_limit_count = sum(1 for u in usage_rows if u["pct"] >= 100)
    above_warning_count = sum(1 for u in usage_rows if u["pct"] >= 70)
    usage_footer = {
    ...
    return templates.TemplateResponse(request, "stats/ot_dashboard.html", _ctx(
        ...
        **task_access_ctx,
        ...
        months=months,
        team_filter=effective_team if can_filter_team else "",
        can_filter_team=can_filter_team,
        effective_team_label=effective_team_label,
        teams=teams,
        summary=summary,
        usage_list=usage_list,
        usage_scope_count=usage_scope_count,
