# ERP Implementation Plan v2 (SSOT v2.0 MiniPatch 1~12b-fix3 Aligned)

- 기준 SSOT: **ERP 통합 SSOT v2.0 (MiniPatch 1~12b-fix3 Applied)** — 2026-03-10 (Asia/Singapore)
- 기준 Plan: `ERP_SSOT_v2_Implementation_Plan_v2_MiniPatch1-10_Aligned_2026-03-03.md`
- 본 문서 목적: 위 SSOT 기준으로 **브랜치/커밋 단위 실행 플랜을 MiniPatch 11~12b-fix3 변경사항까지 포함해 재정렬/수정**한다.
...
> - Settings/Mobile(12b-fix3): **automatic week advancement 구현**(`meeting_auto_advance`, `snapshot_advance_day`, `snapshot_advance_time`) + **mobile OT O2 pagination canonicalized** 반영
...
- system_config 기본 key들 seed:
  - `meeting_current_date`, `meeting_auto_advance`, `snapshot_advance_day`, `snapshot_advance_time`, `needs_update_threshold_hours`(기본 72)
...
- Export는 모바일에서 제외하고, pagination은 mobile O2에서 지원
...
- Snapshot Week Config
  ...
  - automatic week advancement는 Monday-fixed working week, Asia/Singapore timezone, day+time trigger, missed-week catch-up 기준으로 동작
...
- task export는 저장된 `meeting_current_date` 기준
...
- MiniPatch 12b-fix3 반영:
  - System Settings에 automatic week advancement 구현 반영
  - `meeting_auto_advance`, `snapshot_advance_day`, `snapshot_advance_time` canonical key 반영
  - mobile OT O2 pagination을 canonical mobile behavior로 반영
...
- [x] 문서 제목 / 기준 SSOT가 **MiniPatch 1~12b-fix3**로 갱신됨
