# ERP 통합 SSOT v2.0 (MiniPatch 1~12b-fix3 Applied)
**MH Tracking (OT) + Task Manager (Weekly Snapshot) 통합 단일 기준 문서**

- 문서 버전: v2.0 (Audit-Patched, MiniPatch 1~12b-fix3 Applied)
- 기준일: 2026-03-10 (Asia/Singapore)
- 기반 문서:
...
- 변경사항 요약:
  ...
  - MiniPatch 1~12b-fix3 반영 (MiniPatch 1~9 + MiniPatch 10: 감사 결함 7건 수정 + MiniPatch 11: RFO No.(work_packages.rfo_no) + CSV Import(Reference Data) + 월 72h OT 한도(제출 차단/위젯) + 2단계 OT 승인(PENDING→ENDORSED→APPROVED) + Task-RFO 연결(task_items.work_package_id) + MiniPatch 12: Task Distribution/Worker Assignment/Planned MH + Task Manager·Data Entry 역할 분리 + RFO Detail Lean Metrics + OT by Reason/Weekly Trend + 모바일 최적화 + MiniPatch 12b: 모바일 셸 3탭(Tasks/OT/More) + Worker 모바일 접근 명확화 + OT 통합 탭(O1/O2/O3) + 더보기 탭(RFO 요약/도움말/글자 크기/내 계정/로그아웃) + M5 작업 상세 + 모바일 접근성 규칙 + MiniPatch 12b-fix1: D11 NEEDS UPDATE 배지 기준 복원 + `needs_update_threshold_hours`(기본 72h) seed + Settings 설명 보강 + MiniPatch 12b-fix2: §6.3 Data Entry RBAC 경계 복원(Init-week/Soft delete → Task Manager 전용, Save & Next 추가) + §9.5 full-screen modal 전환 규칙 복원 + MiniPatch 12b-fix3: automatic week advancement 및 mobile O2 pagination canonicalization)

- MiniPatch 12b-fix3 반영
  - System Settings에 automatic week advancement를 정식 기능으로 반영 (`meeting_auto_advance`, `snapshot_advance_day`, `snapshot_advance_time`)
  - automatic week advancement는 `manual | scheduled`, Monday-fixed working week, Asia/Singapore timezone, day+time trigger, missed-week catch-up을 기준으로 동작
  - mobile OT `O2`는 status filter + pagination을 지원하고, export만 모바일에서 제외하는 것으로 기준을 갱신
  - Task surfaces와 task export는 계속 저장된 `meeting_current_date`를 단일 기준으로 사용
...
Admin Settings 화면(회의 기준일, automatic week advancement, badge threshold, 알림 토글/수신자/템플릿 등)을 위한 설정 저장/조회 API.
...
- Snapshot week 관련 canonical keys:
  - `meeting_current_date`
  - `meeting_auto_advance` = `manual | scheduled`
  - `snapshot_advance_day` = `monday .. sunday`
  - `snapshot_advance_time` = `HH:MM` (24-hour, Asia/Singapore)
- automatic week advancement가 활성화되어도 task surfaces와 task export는 저장된 `meeting_current_date`를 단일 기준으로 사용한다.
...
| System Settings | /admin/settings | ADMIN | **Snapshot Week Config** + **Automatic week advancement** + **Data Entry badge threshold** + 알림/수신자/템플릿 설정 + saved-week task export | **수정** |
...
| Settings | "Snapshot Week Configuration": `meeting_current_date` 수동 조정 + `meeting_auto_advance` / `snapshot_advance_day` / `snapshot_advance_time` 기반 automatic week advancement + `needs_update_threshold_hours` (기본 72h) + saved-week task export |
...
- Export는 모바일에서 제외하고, pagination은 모바일 O2에서 지원한다.
...
7. **NEEDS UPDATE 배지**: `supervisor_updated_at`이 `system_config['needs_update_threshold_hours']`(기본 72h)보다 오래되면 Data Entry에서 노란 배지로 표시되고, threshold 변경 시 판정이 즉시 반영되는지 확인
8. **RFO Metrics API**: `/api/rfo/{id}/metrics`, `/blockers`, `/worker-allocation`, `/burndown` 응답 구조 검증
9. **OT Stats 확장**: `/api/stats/ot-by-reason`, `/api/stats/ot-weekly-trend` 권한/집계 검증
