# ERP 통합 SSOT v2.0 (MiniPatch 1~12b-fix2 Applied)
**MH Tracking (OT) + Task Manager (Weekly Snapshot) 통합 단일 기준 문서**

- 문서 버전: v2.0 (Audit-Patched, MiniPatch 1~12b-fix2 Applied)
...
- 변경사항 요약:
  ...
  - MiniPatch 1~12b-fix2 반영 (MiniPatch 1~9 + MiniPatch 10: 감사 결함 7건 수정 + MiniPatch 11: RFO No.(work_packages.rfo_no) + CSV Import(Reference Data) + 월 72h OT 한도(제출 차단/위젯) + 2단계 OT 승인(PENDING→ENDORSED→APPROVED) + Task-RFO 연결(task_items.work_package_id) + MiniPatch 12: Task Distribution/Worker Assignment/Planned MH + Task Manager·Data Entry 역할 분리 + RFO Detail Lean Metrics + OT by Reason/Weekly Trend + 모바일 최적화 + MiniPatch 12b: 모바일 셸 3탭(Tasks/OT/More) + Worker 모바일 접근 명확화 + OT 통합 탭(O1/O2/O3) + 더보기 탭(RFO 요약/도움말/글자 크기/내 계정/로그아웃) + M5 작업 상세 + 모바일 접근성 규칙 + MiniPatch 12b-fix1: D11 NEEDS UPDATE 배지 기준 복원 + `needs_update_threshold_hours`(기본 72h) seed + Settings 설명 보강 + MiniPatch 12b-fix2: §6.3 Data Entry RBAC 경계 복원(Init-week/Soft delete → Task Manager 전용, Save & Next 추가) + §9.5 full-screen modal 전환 규칙 복원)
